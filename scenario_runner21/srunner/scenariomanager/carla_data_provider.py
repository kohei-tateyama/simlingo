...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        ## Added - my modification
      
        if autopilot:
            if isinstance(actor, carla.Vehicle):
                actor.set_autopilot(autopilot, CarlaDataProvider._traffic_manager_port)
                
                print(f"[DEBUG][LHT] Single actor spawn: {actor.type_id}")
                print(f"[DEBUG][LHT] Has _is_lht_map: {hasattr(CarlaDataProvider, '_is_lht_map')}")
                
                
                if hasattr(CarlaDataProvider, '_is_lht_map'):
                    print(f"[DEBUG][LHT] _is_lht_map value: {CarlaDataProvider._is_lht_map}")
                
                # Apply LHT-specific Traffic Manager settings if on LHT map
                if hasattr(CarlaDataProvider, '_is_lht_map') and CarlaDataProvider._is_lht_map:
                    print(f"[DEBUG][LHT] Applying LHT to single actor {actor.type_id}...")
                    print(f"[DEBUG][LHT] Single actor spawn: {actor.type_id}")
                    print(f"[DEBUG][LHT] Has _is_lht_map: {hasattr(CarlaDataProvider, '_is_lht_map')}")
                    try:
    ...
    def request_new_actors(actor_list, attribute_filter=None, tick=True):
        ...
        for actor in actors:
            ...
            # Apply LHT settings to batch-spawned vehicles with autopilot
            if hasattr(CarlaDataProvider, '_is_lht_map') and CarlaDataProvider._is_lht_map:
    ...
    def request_new_batch_actors(model, amount, spawn_points, autopilot=False,
        ...
        batch = []
        spawn_transforms = []  # Store transforms for later registration
        ...
        for i in range(amount):
            ...
            if spawn_point:
                batch.append(SpawnActor(blueprint, spawn_point).then(
                    SetAutopilot(FutureActor, autopilot, CarlaDataProvider._traffic_manager_port)))
                spawn_transforms.append(spawn_point)  # Store the transform

        ## Added - my modification
        actors = CarlaDataProvider.handle_actor_batch(batch, tick)

        print(f"[DEBUG][LHT] handle_actor_batch returned {len(actors)} actors")
        print(f"[DEBUG][LHT] CarlaDataProvider has _is_lht_map: {hasattr(CarlaDataProvider, '_is_lht_map')}")
        if hasattr(CarlaDataProvider, '_is_lht_map'):
            print(f"[DEBUG][LHT] _is_lht_map value: {CarlaDataProvider._is_lht_map}")

        for i, (actor, spawn_transform) in enumerate(zip(actors, spawn_transforms)):
            if actor is None:
                print(f"[DEBUG][LHT] Actor {i} is None, skipping")
                continue
                
            print(f"[DEBUG][LHT] Processing actor {i}: {actor.type_id}")
            
            CarlaDataProvider._carla_actor_pool[actor.id] = actor
            CarlaDataProvider.register_actor(actor, spawn_transform)
            
            # Apply LHT settings to batch-spawned vehicles with autopilot
            if hasattr(CarlaDataProvider, '_is_lht_map') and CarlaDataProvider._is_lht_map:
                print(f"[DEBUG][LHT] LHT map detected, checking if vehicle...")
                
                if isinstance(actor, carla.Vehicle):
                    print(f"[DEBUG][LHT] Actor is a vehicle, checking role_name...")
                    role_name = actor.attributes.get('role_name', 'unknown')
                    print(f"[DEBUG][LHT] Role name: {role_name}")
                    
                    if role_name != 'hero':
                        print(f"[DEBUG][LHT] Applying LHT settings to {actor.type_id}...")
                        try:
                            tm = CarlaDataProvider._client.get_trafficmanager(CarlaDataProvider._traffic_manager_port)
                            ...
                            import traceback
                            print(f"[ERROR][LHT] Failed to apply LHT to batch vehicle: {e}")
                            print(traceback.format_exc())
                    else:
                        print(f"[DEBUG][LHT] Skipping hero vehicle")
                else:
                    print(f"[DEBUG][LHT] Actor is not a vehicle: {type(actor)}")
            else:
                print(f"[DEBUG][LHT] Not LHT map or flag not set")
