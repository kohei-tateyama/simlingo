...
# Script to run many data collection jobs via script/run_carla_mp_pilot_script_carla0916.sh
# Usage:
#   ./script/getting_data_training_carla0916.sh [--dry-run]
# Edit arrays below to change the combinations.
# NOTE: This script continues execution even if individual tasks fail.
# Example 
# bash script/run_carla_mp_pilot_script_carla0916.sh --mode autopilot --duration 15 --route urban --town Town03 --weather ClearNoon --spawn-index 42 --fps 20
...
# start=10; step=10; last=300
# DURATIONS=()
# for ((v=start; v<=last; v+=step)); do
#   DURATIONS+=("$v")
# done
...
WRAPPER=script/run_carla_mp_pilot_script_carla0916.sh
...
for route_type in "${ROUTE_TYPES[@]}"; do
    ...
    for spawn_idx in "${SPAWN_INDICES[@]}"; do
        ...
        CMD=(bash "$WRAPPER" --mode autopilot --duration "$duration" --route "$route_type" --fps 20 --town "$town" --spawn-index "$spawn_idx" --weather "$weather" )

        # Print command for logging / review
        TASK_ID="town=$town route=$route_type weather=$weather spawn=$spawn_idx duration=${duration}s"
        echo "[RUN] $TASK_ID"
        echo "      ${CMD[*]}"
