...
export TEAM_CONFIG="data_collection"
export SAVE_PATH=/media/external_ssd/workspace/simlingo/database/simlingo_v4_bosch_2025_01_10/data/simlingo_carla0916_leaderboard_test
# export SAVE_PATH=/workspace/simlingo/database/simlingo_v4_bosch_2025_01_10/data/simlingo_carla0916_2_
export TOWN="Town03"  # Will be overridden by route XML
...
# Ensure leaderboard checkpoint directory exists to avoid FileNotFoundError
if [ -n "${LEADERBOARD_CHECKPOINT:-}" ]; then
    CHKP_DIR=$(dirname "${LEADERBOARD_CHECKPOINT}")
    if [ ! -d "${CHKP_DIR}" ]; then
        info "Creating leaderboard checkpoint directory: ${CHKP_DIR}"
        mkdir -p "${CHKP_DIR}" || warn "Could not create ${CHKP_DIR}"
    fi
fi
