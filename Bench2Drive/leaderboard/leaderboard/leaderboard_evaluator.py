...
class LeaderboardEvaluator(object):
    ...
    # Tunable parameters
    client_timeout = 3600.0  # in seconds (1 hour for slow inference)
    ...
    def _setup_simulation(self, args):
        ...
        # time.sleep(30)
        # ADD THIS CHECK: Skip CARLA spawning if --resume True
        if not args.resume:
            self.carla_path = os.environ["CARLA_ROOT"]
            args.port = find_free_port(args.port)
            cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -nosound -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank}"
            ...
            print(f"Connecting to existing CARLA server on port {args.port}...", flush=True)
            self.server = None # No server process to manage
