...
# Additional imports for bounding box processing
try:
...
def convert_rgb_to_names(rgb_tuple):
    """Convert RGB tuple to closest CSS color name"""
    if not WEBCOLORS_AVAILABLE:
        # Fallback: simple color name approximation
        r, g, b = rgb_tuple
...
def get_relative_transform(ego_matrix, vehicle_matrix):
    """Compute relative transform from ego to vehicle"""
    relative_matrix = np.linalg.inv(ego_matrix) @ vehicle_matrix
    return relative_matrix[:3, 3]


def get_trafficlight_trigger_location(traffic_light):
    """Get trigger location for traffic light"""
    base_transform = traffic_light.get_transform()
    base_location = traffic_light.get_location()
...
class JapaneseStyleAutopilot:
    def __init__(self, autopilot=False, duration=60, route_type='highway', port_localhost=2000, port_traffic=8000, town='Town13', fps=20.0, num_imgs_per_frame=6, callback_debug=False, weather='SoftRainNight',spawn_idx=None):
        ...
        # Get world map for waypoint queries
        self.world_map = self.world.get_map()
        
        # Create minimal config object for bb_save_radius
        class MinimalConfig:
            bb_save_radius = 50.0  # meters
            point_format = 3
        ...
        # Track traffic manager reference
        self.tm = self.traffic_manager
        
        # Scenario name (for compatibility)
        self.scenario_name = "JapaneseAutopilot"
        
        # For tracking nearby traffic lights and stop signs
    ...
    def get_bounding_boxes(self, lidar=None):
        """
        Get bounding boxes for all nearby actors in the format matching data_agent.
        This method closely follows the structure from team_code/data_agent.py
        """
        results = []
        ...
        ego_wp = self.world_map.get_waypoint(self.player_vehicle.get_location(), project_to_road=True, lane_type=carla.libcarla.LaneType.Any)
        
        # Compute lane info for ego
        ...
        # Analyze lanes to left and right
        ...
        aff_vehicle_id_10 = None
        aff_vehicle_id_15 = None
        aff_vehicle_id_20 = None
        aff_vehicle_id_40 = None

        try:
            next_action = self.tm.get_next_action(self.player_vehicle)[0]
        except:
            next_action = None
        ...
        vehicle_list = _actors.filter('*vehicle*')

        for vehicle in vehicle_list:
            if vehicle.get_location().distance(self.player_vehicle.get_location()) < self.config.bb_save_radius:
        ...
        # Process walkers
        walkers = _actors.filter('*walker*')
        for walker in walkers:
            if walker.get_location().distance(self.player_vehicle.get_location()) < self.config.bb_save_radius:
        ...
        # Process traffic lights for VQA
        traffic_lights = self.get_nearby_object(ego_transform.location, _actors.filter('*light*'), self.config.bb_save_radius)
        ...
        for traffic_light in traffic_lights:
            ...
            relative_yaw = normalize_angle(yaw - ego_yaw)
            relative_pos = get_relative_transform(ego_matrix, traffic_light_matrix)
        ...
        # Process stop signs for VQA  
        stop_signs = self.get_nearby_object(ego_transform.location, _actors.filter('*stop*'), self.config.bb_save_radius)
        ...
        for stop_sign in stop_signs:
            ...
            relative_yaw = normalize_angle(yaw - ego_yaw)
            relative_pos = get_relative_transform(ego_matrix, stop_sign_matrix)
        ...
        statics = _actors.filter('static.*')
        for static in statics:
            if static.get_location().distance(self.player_vehicle.get_location()) < self.config.bb_save_radius:
    ...
    def record_data(self):
        ...
        # Build boxes JSON using comprehensive method matching data_agent format
    ...
    def _wps_next_until_lane_end(self, wp):
        """Get waypoints until lane end"""
        try:
            next_wps = [wp]
            while len(next_wps) < 200:
                wp_next = next_wps[-1].next(1.0)
                if len(wp_next) == 0:
                    break
                if wp_next[0].lane_id * next_wps[-1].lane_id < 0:
                    break
                if wp_next[0].road_id != next_wps[-1].road_id:
                    break
                next_wps.append(wp_next[0])
            return next_wps
        except:
            return [wp]
    
    def get_nearby_object(self, location, actor_list, radius):
        """Get actors within radius of location"""
        nearby = []
    ...
    def _get_forward_speed(self, transform=None, velocity=None):
        """Calculate forward speed from velocity vector"""
        if velocity is None:
            return 0.0
    ...
    def get_points_in_bbox(self, vehicle_pos, vehicle_yaw, extent, lidar):
        """Count LiDAR points within bounding box"""
        try:
            ...
            # Translate and rotate lidar points
            lidar_transformed = lidar[:, :3] - vehicle_pos
            lidar_rotated = lidar_transformed @ rot_matrix.T
            
            # Check if points are within bbox
            in_bbox = (
                (np.abs(lidar_rotated[:, 0]) <= extent[0]) &
                (np.abs(lidar_rotated[:, 1]) <= extent[1]) &
                (np.abs(lidar_rotated[:, 2]) <= extent[2])
            )
            
            return int(np.sum(in_bbox))
        except:
            return 0
