...
class CARLA_Data(Dataset):
    ...
    def __init__(self,
        ...
        route_dirs = glob.glob(self.data_path + '/**/data/simlingo/**/Town*', recursive=True)
        ...
        for sub_root in tqdm(route_dirs, file=sys.stdout):
            ...
            # Only count .jpg files, not folders
            num_seq = len([f for f in os.listdir(route_dir + '/rgb') if f.endswith('.jpg')])
            ...
            for seq in range(skip_first_n_frames, num_seq - self.pred_len - self.hist_len):
                ...
                # Validate that ALL required measurement files exist (current + future frames)
                # Dataloader needs measurements from seq to seq+hist_len+pred_len-1
                skip = False
                for idx in range(self.hist_len + self.pred_len):
                    measurement_file = route_dir + '/measurements' + f'/{(seq + idx):04}.json.gz'
...
def main():
    ...
    data_path = '/media/external_ssd/workspace/simlingo/database/simlingo_v4_bosch_2025_01_10'
    save_path = '/media/external_ssd/workspace/simlingo/database/bucketsv4_simlingo'
